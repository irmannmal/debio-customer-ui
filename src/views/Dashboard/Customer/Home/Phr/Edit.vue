...
export default {
  ...
  methods: {
    ...
    async prepareData() {
      try {
        ...
        for (const file of files) {
          const details = await getIpfsMetaData(
            file.recordLink.split("/").pop()
          );
          ...
          completeFiles.push({
            ...file,
            file: new File([blobData], details.rows[0].metadata.name, {
              type: "application/pdf"
            }),
            oldFile: new File([blobData], details.rows[0].metadata.name, {
              type: "application/pdf"
            }),
    ...
    handleNewFile() {
      ...
      fr.onload = async function() {
        try {
          ...
          if (context.isEdit) {
            const index = context.phr.files.findIndex(
              (phrFile) => phrFile.id === id
            );
    ...
    handleModalPassword() {
      ...
      const isPHRValid = Object.values(this.isDirty?.phr).every(
        (v) => v !== null && v === false
      );
    ...
    async calculateTxWeight() {
      ...
      const txWeight = await registerElectronicMedicalRecordFee(
        this.api,
        this.wallet,
        this.phr
      );
